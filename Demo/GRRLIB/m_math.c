...
unsigned int m_rand_user(m_rand_desc *desc)
{
...
float m_randf_user(m_rand_desc *desc)
{
...
void m_quat_identity(float4 *dest)
{
...
void m_quat_normalize(float4 *dest, const float4 *src)
{
...
void m_quat_rotation_axis(float4 *dest, const float3 *axis, float angle)
{
   float ha = angle * 0.5f;
   float sina = sinf(ha);
   float cosa = cosf(ha);
...
void m_quat_rotation_euler(float4 *dest, const float3 *euler)
{
...
void m_quat_mul(float4 *dest, const float4 *A, const float4 *B)
{
...
void m_quat_slerp(float4 *dest, const float4 *A, const float4 *B, float mu)
{
   float4 C;
...
void m_quat_extract_axis_angle(float3 *axis, float *angle, const float4 *quat)
{
   float a, s;
...
void m_mat4_identity(float *dest)
{
...
void m_mat4_perspective(float *dest, float fov, float ratio, float znear, float zfar)
{
   float ymax, xmax;
...
void m_mat4_ortho(float *dest, float left, float right, float bottom, float top, float znear, float zfar)
{
...
void m_mat4_lookat(float *dest, const float3 *pos, const float3 *dir, const float3 *up)
{
...
void m_mat4_translation(float *dest, const float3 *translation)
{
...
void m_mat4_rotation_axis(float *dest, const float3 *axis, float angle)
{
...
void m_mat4_rotation_euler(float *dest, const float3 *euler)
{
...
void m_mat4_scale(float *dest, const float3 *scale)
{
   dest[0] = scale->x;
   dest[5] = scale->y;
   dest[10] = scale->z;
...
void m_mat4_mul(float *dest, const float *A, const float *B)
{
...
void m_mat4_transpose(float *dest, const float *src)
{
...
void m_mat4_inverse_transpose(float *dest, const float *src)
{
   float tmp[12];
   float det;
...
void m_mat4_inverse(float *dest, const float *src)
{
   float tmp[16];
...
void m_mat4_extract_euler(float3 *dest, const float *matrix)
{
...
void m_mat4_rotate3(float3 *dest, const float *matrix, const float3 *src)
{
...
void m_mat4_inverse_rotate3(float3 *dest, const float *matrix, const float3 *src)
{
...
void m_mat4_transform3(float3 *dest, const float *matrix, const float3 *src)
{
...
void m_mat4_transform4(float4 *dest, const float *matrix, const float4 *src)
{
